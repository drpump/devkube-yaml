...
I'm feeling uncomfortable because my dev environment really doesn't look much like
production.

So I want to set up a dev environment using kubernetes. In the cloud.
...
My code changes also need to persist so I can check them into git later, once
...
To build an editable dev environment, we need at least three pieces:

1. *An application runtime container*. For simplicity, I'm using (nginx)[http://nginx.org].
   Later on, once I've got editing sorted, I'll tackle something a bit more substantial,
   maybe rails or django.
2. *An editing/coding container*. For now, I'm using an
   (Alpine Linux)[https://alpinelinux.org] instance with vim. We can build from there.
3. *Shared, persistent storage*. Persistent storage is a bit of a pain with containers
...
partly because I can run it as a cluster internal service (private), and partly because I
...
To make NFS available to other pods in the cluster, we also need to create a kubernetes service
...
* The `selector` defines the label that kubernetes will use to find service instances, in this
  ...
  the service won't be able to find the instance. You might see other configurations use `app:`
...
* If DNS is enabled in your cluster (enabled by default in Digital Ocean),
  ...
  DNS is not available to `kubelet` so it can't resolve the name. So to mount the NFS share in a pod,
  we need to inject (edit) the NFS server IP address into configuration before deploying our dependent
  pods. If you use the hostname, the deployment of those pods will repeatedly fail trying to mount the
  NFS volume.
...
(e.g. `/html/`) to limit the scope.

Our nginx should now be running, and now we need to define a service so that it is accessible:
```
apiVersion: v1
kind: Service
metadata:
...
* The `nodePort: 30080` tells kubernetes to make the service accessible on port `30080`. Kubernetes will
  ...
  If you don't speciy a nodePort, kubernetes will allocate one automatically and you can use `kubectl get services`
  to find it.

So if we have a node with the IP address `245.122.10.11` allocated to our cluster, we can navigate to
`http://245.122.10.11:30080/` to access our app.
...
also asked the instance to run a loooong sleep command to keep it operating while we add content (otherwise the
container would exit immediately).

To add content, we need to connect to this instance. In future I'll likely use an sshd/rsync container so I
...
Now if you browse to your exposed app server address, this content should appear in your browser.

## Wrapping up

We've created a simple, editable development environment using kubernetes in the cloud. Our code is stored
...
* We need to add databases, cache servers etc to provide a realistic environment. For development,
  ...
  services like RDS, snowflake etc in production. The containers can also the NFS share if desired,
...
* I want to be able to develop on my iPad. With this setup I'm getting close, but lsyncd is
  ...
  when it comes to searching across files (e.g. find me the definition of that class/function).
...
* Package management is likely to require some thought when using ruby/python/nodejs app servers. As a
  ...
  NFS share rather than read-only in a container image.
